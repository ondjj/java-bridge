...
public class BridgeExpressionMaker {
    ...
    public String createBridgeExpression(List<String> bridge, List<String> userBridge) {
        ...
        for (int i = 0; i < userBridge.size(); i++) {
            ...
            String userBridgeExpression = bridge.get(i);
    ...
    private String createUpperBridgeExpression(String bridgeExpression, String userBridgeExp) {
        if (bridgeExpression.equals(BRIDGE_DOWN_EXPRESSION)) {
            return BRIDGE_EMPTY_EXPRESSION;
        }
        return createBridgeCorrectiveExpression(bridgeExpression, userBridgeExp);
    }

    private String createLowerBridgeExpression(String bridgeExpression, String userBridgeExp) {
    ...
    private String createBridgeCorrectiveExpression(String bridgeExpression, String userBridgeExp) {
        if (bridgeExpression.equals(userBridgeExp)) {
            return BRIDGE_CORRECT_EXPRESSION;
        }
        return BRIDGE_INCORRECT_EXPRESSION;
    }

    private String createWholeBridgeExpression(List<String> upperBridgeExpressions,
                                               List<String> lowerBridgeExpressions) {
        return createSingleBridgeExpression(upperBridgeExpressions)
                + LINE_SEPARATOR
                + createSingleBridgeExpression(lowerBridgeExpressions);
    }

    private String createSingleBridgeExpression(final List<String> bridgeExpressions) {
